...
def step(
    ...
    sns.scatterplot(data=data, x=x, y=y, color=color, s=15)
...
def roc_curve(
    ...
    prob_name = argname(arg='y_prob')

    plt.plot(
        fpr,
        tpr,
        color=color,
        label=f'AUC({prob_name}): {auc_value:.4f}',
        linewidth=1.0,
    )
...
def biplot(
    ...
    plt.axvline(x=0, color='0.5', linestyle='--', linewidth=0.5)
    plt.axhline(y=0, color='0.5', linestyle='--', linewidth=0.5)
    ...
    plt.xlabel(xlabel=f'PC{x}')
    plt.ylabel(ylabel=f'PC{y}')
    plt.show()
